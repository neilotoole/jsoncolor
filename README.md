...
[`MarshalIndent`](https://pkg.go.dev/github.com/neilotoole/jsoncolor#MarshalIndent) now indents inline, and the documentation is brought up to date.
There are no exported API changes and no change to encoder output.

#### Changed

- [#77](https://github.com/neilotoole/jsoncolor/issues/77): [`MarshalIndent`](https://pkg.go.dev/github.com/neilotoole/jsoncolor#MarshalIndent) now indents inline, using the same [`Indenter`](https://pkg.go.dev/github.com/neilotoole/jsoncolor#Indenter) as [`Encoder.SetIndent`](https://pkg.go.dev/github.com/neilotoole/jsoncolor#Encoder.SetIndent), instead of marshaling compact and re-indenting the result with [`Indent`](https://pkg.go.dev/github.com/neilotoole/jsoncolor#Indent). Output is byte-identical to before, including `MarshalIndent(v, "", "")`, which keeps [`encoding/json`](https://pkg.go.dev/encoding/json)'s line-breaking behavior rather than the [`Encoder`](https://pkg.go.dev/github.com/neilotoole/jsoncolor#Encoder)'s disable-on-empty rule; `TestMarshalIndent_Parity` pins it against the previous algorithm across the encode corpus. Measured with `BenchmarkMarshalIndent` on an Apple M1 Max under Go 1.26.5 (`-count=10`, benchstat): 59% faster on the decoded `sakila_actor.json` document and 50% faster on a slice of 1,000 synthetic records, allocating about half the bytes.
- [#75](https://github.com/neilotoole/jsoncolor/issues/75): The fixed ANSI reset that closes every colorized token is now documented, on [`Color`](https://pkg.go.dev/github.com/neilotoole/jsoncolor#Color) and [`Colors`](https://pkg.go.dev/github.com/neilotoole/jsoncolor#Colors) and in a README "Color reset" section: why output is not safe to nest inside an already-styled region, and why an attribute-specific closer cannot be computed from an opaque prefix. Documentation only.
...
- [#53](https://github.com/neilotoole/jsoncolor/issues/53): [`Colors.TextMarshaler`](https://pkg.go.dev/github.com/neilotoole/jsoncolor#Colors.TextMarshaler) falls back to [`Colors.String`](https://pkg.go.dev/github.com/neilotoole/jsoncolor#Colors.String) when unset, the same way the granular punctuation fields fall back to [`Colors.Punc`](https://pkg.go.dev/github.com/neilotoole/jsoncolor#Colors.Punc). Previously an unset field produced an uncolored token followed by a stray ANSI reset.
- [#54](https://github.com/neilotoole/jsoncolor/issues/54): An empty [`Color`](https://pkg.go.dev/github.com/neilotoole/jsoncolor#Color) now emits neither a prefix nor a reset, as the [`Color`](https://pkg.go.dev/github.com/neilotoole/jsoncolor#Color) docs always promised. `&Colors{}` produces output identical to a nil palette, and [`DefaultColors()`](https://pkg.go.dev/github.com/neilotoole/jsoncolor#DefaultColors) no longer emits a reset after every punctuation mark.
- [#56](https://github.com/neilotoole/jsoncolor/issues/56): A nil embedded struct pointer no longer leaves a double or trailing comma behind, and an indented struct whose every field is omitted encodes as `{}`, matching [`encoding/json`](https://pkg.go.dev/encoding/json).
- [#58](https://github.com/neilotoole/jsoncolor/issues/58): A failed [`Encode`](https://pkg.go.dev/github.com/neilotoole/jsoncolor#Encoder.Encode) no longer leaks indenter depth into later [`Encode`](https://pkg.go.dev/github.com/neilotoole/jsoncolor#Encoder.Encode) calls on the same [`Encoder`](https://pkg.go.dev/github.com/neilotoole/jsoncolor#Encoder).
...
- [#66](https://github.com/neilotoole/jsoncolor/issues/66): A trusted [`RawMessage`](https://pkg.go.dev/github.com/neilotoole/jsoncolor#RawMessage) that is not well-formed JSON no longer leaks indenter depth into later [`Encode`](https://pkg.go.dev/github.com/neilotoole/jsoncolor#Encoder.Encode) calls. End of input inside a container is now reported as a syntax error internally, so the verbatim fallback starts from a clean depth.
- [#67](https://github.com/neilotoole/jsoncolor/issues/67): A nil `[]byte` is now a single null token colored by [`Colors.Null`](https://pkg.go.dev/github.com/neilotoole/jsoncolor#Colors.Null), instead of a null nested inside the [`Colors.Bytes`](https://pkg.go.dev/github.com/neilotoole/jsoncolor#Colors.Bytes) prefix with two resets.
...
Taken together, the changes in this release alter encode time against v0.9.1 as follows, measured with `BenchmarkCmp` on an Apple M1 Max (Go 1.26, interleaved runs, benchstat). The colorless and colored columns use a nil palette and [`DefaultColors()`](https://pkg.go.dev/github.com/neilotoole/jsoncolor#DefaultColors) respectively. The segmentio upstream reference cells did not move.
