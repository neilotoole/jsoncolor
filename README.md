...
- [#78](https://github.com/neilotoole/jsoncolor/pull/78): README overhaul. The stale caveat that [`IsColorTerminal`](https://pkg.go.dev/github.com/neilotoole/jsoncolor#IsColorTerminal) "will fail inside Goland" is replaced, in the README and both examples, with the actual condition: stdout is not a terminal. The 2021 benchmark table is regenerated on current hardware and Go; the Notes section is dissolved into Punctuation, Detecting color support, and a new "Relationship to [`encoding/json`](https://pkg.go.dev/encoding/json)" section; and the dead linter/porting note is removed.
